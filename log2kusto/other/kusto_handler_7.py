...
Idea is to make run self.flush_writes() through self.flush() without event loop
(using asyncio.run(...) syntax) and wait until its completion before running 
other functions
...
'''
print('importing KustoHandler from kusto_handler_7.py')
