...
Idea is to run self.flush_writes() through self.flush() (with asyncio.run(...),   
not event loop, and wait until completion before running other functions. Also,
self.flush() would be called before exit without dealing with other operations 
...
'''
print('importing KustoHandler from kusto_handler_8.py')
